...
class Version:
    ...
    def __init__(self, major=0, minor=0, patch=0, release='dev', release_num=0):
        self.major = major
        self.minor = minor
        self.patch = patch
    ...
    def __repr__(self):
        return (f"Version(major={self.major}, minor={self.minor}, patch={self.patch}, "
                f"release='{self.release}', release_num={self.release_num})")

    def __str__(self):
        release_str = self.RELEASE_MAP.get(self.release, "Unknown Release")
        release_num_str = f' #{self.release_num}' if self.release_num > 0 else ''

        return f"{self.major}.{self.minor}.{self.patch} {release_str}{release_num_str}"
    ...
    def from_dict(cls, version_dict):
        return cls(
                ...
                release=version_dict.get('release', 'dev'),
                release_num=version_dict.get('release_num', 0)
                )
    ...
    def from_string(cls, version_str):
        ...
        if len(parts) > 1:
            ...
            release = release_parts[0]
            release_num = int(release_parts[1]) if len(release_parts) > 1 else 0

        return cls(major, minor, patch, release, release_num)
