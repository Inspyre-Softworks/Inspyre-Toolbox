...
class Numerical :
    ...
    def count_noun(self, *args, **kwargs) :
        if self.noun is None :
            if 'noun' in kwargs.keys() :
                return self.__count_noun(*args, **kwargs)
            else :
                raise NUM_ERR.NotANumberError(
                        "No noun in object instance. Please try again while providing a value "
                        "for the"
                        )
